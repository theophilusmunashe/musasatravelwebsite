...
/* ─── Main Export ────────────────────────────────────────────────────── */
export default function ShuttleClient({ transfers }: { transfers: TransferService[] }) {
  const [activeFilter, setActiveFilter] = useState<ServiceType>("all");
  const filtered = activeFilter === "all" ? transfers : transfers.filter((s) => s.type === activeFilter);
  ...
  return (
    <div className="bg-[#0a0a0a] min-h-screen">
      ...
      <section className="py-20 px-6 md:px-16 max-w-7xl mx-auto">
        ...
        {filtered.length === 0 && (
          <p className="text-center py-20 text-white/40 text-lg">No transfers in this category yet.</p>
        )}
      </section>
